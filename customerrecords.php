...
$sql1 = "SELECT DISTINCT city FROM Owners";
...
<head>
    ...
    <script>
        $.get("nav.html", function(data){
            ...
            document.getElementById("customers").style.color = "#E14761";
        });
...
<body>
    ...
    <form actions="" method="POST">
        NIC Number<input type="text" name="id" placeholder="18237410923874">&ensp;&ensp;&ensp;&ensp;
    ...
    ?>
    <table class="records">
        <tr><th>NIC</th><th>name</th><th>phone</th><th>email</th><th>address</th></tr>

        <?php 
            if($result->num_rows > 0){
                while($row = $result->fetch_assoc()){
                    echo "<tr><td>".$row["nic"]."</td><td>".$row["name"]."</td><td>".$row["phone"]."</td><td>".$row["email"]."</td><td>".$row["res_number"].", ".$row['street_num'].", ".$row['city']."</td></tr>";
                }
            }
        ?>
